...
import { getInitialIsOnlyChatInSession } from "../../../../sessions/[sessionId]/chats/[chatId]/only-chat-in-session";
import { SessionChatContent } from "../../../../sessions/[sessionId]/chats/[chatId]/session-chat-content";
import { SessionChatProvider } from "../../../../sessions/[sessionId]/chats/[chatId]/session-chat-context";

export const maxDuration = 120;

interface PlatformChatPageProps {
...
export async function generateMetadata({
  params,
}: PlatformChatPageProps): Promise<Metadata> {
  const { sessionId } = await params;
  const sessionRecord = await getSessionByIdCached(sessionId);

  return {
    title: `${sessionRecord?.title ?? "Чат"} · ИИ-ХАБ`,
    description: "ИИ-ХАБ ГК ССК — рабочее пространство агента.",
  };
}

export default async function PlatformChatPage({
  params,
}: PlatformChatPageProps) {
